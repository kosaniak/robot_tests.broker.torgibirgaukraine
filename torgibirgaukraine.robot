...
Library  String
Library  DateTime
Library  torgibirgaukraine_service.py
...
Підготувати клієнт для користувача
  ...
  Set Global Variable   ${TORGIBIRGAUKRAINE_LOT_ID}   ${EMPTY}
  Set Global Variable   ${TORGIBIRGAUKRAINE_MODIFICATION_DATE}   ${EMPTY}
  ...
  Run Keyword If  '${username}' != 'torgibirgaukraine_Viewer'  Login  ${username}
...
Створити тендер
  ...
  ${guarantee}=           Convert To String   ${guarantee}
   ...
   ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
   ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _publish_lot
   ...
   Wait Until Element Is Visible   xpath=//a[contains(@href,'${TORGIBIRGAUKRAINE_LOT_ID}')]   15
   Click Link   xpath=//a[contains(@href,'${TORGIBIRGAUKRAINE_LOT_ID}')]
...
Перейти в розділ додавання активів
  ...
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _edit_lot
  Клацнути по випадаючому списку   ${drop_id}
  Виконати дію   ${action_id}
...
Пошук тендера по ідентифікатору
    ...
    ${flag}=  Run Keyword And Return Status  Should Be Empty  ${TORGIBIRGAUKRAINE_LOT_ID}
...
Пошук тендера у разі наявності змін
  ...
  ${status}=   Run Keyword And Return Status   Should Not Be Equal   ${TORGIBIRGAUKRAINE_MODIFICATION_DATE}   ${last_mod_date}
  Run Keyword If   ${status}   torgibirgaukraine.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Set Global Variable   ${TORGIBIRGAUKRAINE_MODIFICATION_DATE}   ${last_mod_date}

Завантажити документ в тендер з типом
  [Arguments]  ${username}  ${tender_uaid}  ${filepath}  ${documentType}
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _add_files
  Клацнути по випадаючому списку   ${drop_id}
  Виконати дію   ${action_id}
  Wait Until Element Is Visible   xpath=//input[@type='submit']   10
  ${inputID}=   convert_torgibirgaukraine_string_to_common_string   ${documentType}
...
Отримати кількість предметів в тендері
    ...
    torgibirgaukraine.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
...
Чи фінасова процедура
  ...
  ${procedureType}=   convert_torgibirgaukraine_string_to_common_string   ${procedureType}
...
Подати цінову пропозицію
  ...
  torgibirgaukraine.Пошук тендера по ідентифікатору   ${username}   ${auction_id}
...
Завантажити документ в ставку
  ...
  ...      ${ARGUMENTS[1]} ==  ${filepath}
  ...      ${ARGUMENTS[2]} ==  ${TENDER_UAID}
  torgibirgaukraine.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[2]}
...
Оновити сторінку з тендером
  ...
  ...      ${ARGUMENTS[1]} =  ${TENDER_UAID}
  Switch Browser   ${BROWSER_ALIAS}
  Go To  ${BROKERS['torgibirgaukraine'].syncpage}
  Go To  ${BROKERS['torgibirgaukraine'].homepage}
  torgibirgaukraine.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію із тендера
  ...
  Run Keyword If    "${resp}" == "False"   torgibirgaukraine.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію про status
  ...
  ${status}=   convert_torgibirgaukraine_string_to_common_string   ${return_value}
  [return]   ${status}
...
Отримати інформацію про procurementMethodType
  ...
  ${return_value}=   convert_torgibirgaukraine_string_to_common_string   ${return_value}
  [return]   ${return_value}
...
Отримати посилання на аукціон
  ...
  Run Keyword If    ${isAuctionView} == ${False}   torgibirgaukraine.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}  ${ARGUMENTS[1]}
...
Завантажити протокол аукціону
  ...
  torgibirgaukraine.Оновити сторінку з тендером  ${ARGUMENTS[0]}  ${ARGUMENTS[1]}
  Зайти в розділ купую
...
Підтвердити постачальника
   ...
   ...   ${ARGUMENTS[1]} == tender_uaid
   Зайти в розділ кваліфікація
   ${drop_id}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending.payment
   ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _confirm_payment
   Клацнути по випадаючому списку     ${drop_id}
   Виконати дію   ${action_id}
...
Завантажити ілюстрацію
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _add_imgs
  Клацнути по випадаючому списку   ${drop_id}
...
Додати публічний паспорт активу
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _add_assets_link
  Клацнути по випадаючому списку  ${drop_id}
  Виконати дію   ${action_id}
...
Додати офлайн документ
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _add_access_details
  Клацнути по випадаючому списку  ${drop_id}
  Виконати дію   ${action_id}
...
Додати Virtual Data Room
    ...
    Зайти в розділ списку лотів
    ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
    ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _add_vdr
    Клацнути по випадаючому списку  ${drop_id}
...
Отримати інформацію із запитання
    ...
    torgibirgaukraine.Пошук тендера у разі наявності змін   ${TENDER['LAST_MODIFICATION_DATE']}  ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію із документа по індексу
  ...
  ${text}=  convert_torgibirgaukraine_string_to_common_string   ${text}
  [return]  ${text}
...
Отримати інформацію із документа
  ...
  Run Keyword And Return If  '${username}' == 'torgibirgaukraine_Owner' and '${username}' == 'description'   Fail   Опис документа відсутній на юбіз
...
Скасувати закупівлю
  ...
  Зайти в розділ списку лотів
  ${drop_id}=  Catenate   SEPARATOR=   lot_  ${TORGIBIRGAUKRAINE_LOT_ID}
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}  _cancel_lot
  Клацнути по випадаючому списку  ${drop_id}
...
Отримати кількість документів в ставці
  ...
  torgibirgaukraine.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Зайти в розділ кваліфікація
  ${drop_id}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _confirm_protocol
...
Отримати дані із документу пропозиції
  ...
  ${doc_value}=   convert_torgibirgaukraine_string_to_common_string   ${doc_value}
  [return]  ${doc_value}
...
Завантажити документ рішення кваліфікаційної комісії
  ...
  Зайти в розділ кваліфікація
  ${drop_id}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _disqualification
  Wait Until Keyword Succeeds   10 x   20 s   Run Keywords
  ...   Reload Page
  ...   AND   Клацнути по випадаючому списку  ${drop_id}
  ...   AND   Element Should Be Visible   id=${action_id}
...
Дискваліфікувати постачальника
  ...
  Зайти в розділ кваліфікація
  ${verification}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending.verification
  ${payment}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending.payment
  ${active}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _active
  Wait Until Keyword Succeeds   10 x   20 s   Run Keywords
  ...   Reload Page
...
Перейти на форму підписання контракту
  ...
  Зайти в розділ контракти
  ${drop_id}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _publish_contract
  Wait Until Keyword Succeeds   10 x   20 s   Run Keywords
  ...   Reload Page
  ...   AND   Клацнути по випадаючому списку  ${drop_id}
  ...   AND   Element Should Be Visible   id=${action_id}
...
Підтвердити наявність протоколу аукціону
   ...
   Зайти в розділ кваліфікація
   ${drop_id}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending.verification
   ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _confirm_protocol
   Клацнути по випадаючому списку     ${drop_id}
   Виконати дію   ${action_id}
...
Завантажити протокол аукціону в авард
  ...
  Зайти в розділ кваліфікація
  ${drop_id}=  Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _pending.verification
  ${action_id}=   Catenate   SEPARATOR=   ${TORGIBIRGAUKRAINE_LOT_ID}   _uploadprotocol
